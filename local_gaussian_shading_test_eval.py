"""Generate prompted images and evaluate Gaussian Shading extraction."""
...
DEFAULT_LOCAL_TEST_OPTIONS = (
    PROJECT_ROOT / 'options' / 'test' / 'local_test_GS_reverse_distill_UNet_none_adv.yml')
DEFAULT_PROMPTS = (
    'A lighthouse standing above stormy waves at sunset',
    'A red bicycle parked beside a flower-covered stone wall',
    'An astronaut reading a book in a quiet library',
    'A steaming bowl of ramen on a wooden table',
    'A snow-covered mountain reflected in a clear alpine lake',
    'A vintage train traveling through an autumn forest',
    'A glass greenhouse filled with tropical plants',
    'A small sailboat crossing a calm blue bay',
    'A tabby cat sleeping beside a sunny window',
    'A bustling night market illuminated by paper lanterns',
)

def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            'Generate images from prompts with Diffusers UniPC and '
            'test Gaussian-Shading recovery using the OTBD reverse model.'))
    parser.add_argument('--opt', default=str(DEFAULT_LOCAL_TEST_OPTIONS))
    ...
    parser.add_argument(
        '--outdir',
        default=str(
            PROJECT_ROOT / 'outputs' / 'prompted_gaussian_shading'))
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action='store_true',
        help='Regenerate and overwrite cached prompt samples.')
    parser.add_argument(
        '--steps',
        type=int,
        default=15,
...
def generation_cache_path(cache_dir, prompt_record, seed, args, sd_model_id):
    cache_key = {
        'format_version': 1,
    ...
    return cache_dir / f'sample_{prompt_record["prompt_id"]}_{digest}.pt'
...
def main():
    ...
    cache_dir = (
        ...
        else PROJECT_ROOT / 'outputs' / 'prompt_generation_cache')
    ...
    samples = []
    progress_bar = tqdm(
        enumerate(prompt_records),
        total=len(prompt_records),
        unit='sample',
        desc='Prompted Gaussian-Shading evaluation')
    for index, prompt_record in progress_bar:
        prompt = prompt_record['prompt']
        prompt_embedding = prompt_embeddings(
            pipe, [prompt], device, dtype)
        seed = args.seed + index
        cache_path = generation_cache_path(
            cache_dir, prompt_record, seed, args, opt['sd_model_id'])
        (watermark, watermarked_noise, generated_latent, generated_image,
        ...
        sample_dir = (
            output_dir
            / f'sample_{index:04d}_prompt_{prompt_record["prompt_id"]}')
        save_sample_artifacts(
            sample_dir,
            pipe,
            watermarked_noise,
