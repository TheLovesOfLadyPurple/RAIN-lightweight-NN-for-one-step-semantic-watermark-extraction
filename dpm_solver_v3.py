...
class NoiseScheduleVP:
    ...
    def get_sigmas_with_timesteps(self,timesteps):
        low_idx, high_idx, w = np.minimum(np.floor(timesteps),999), np.minimum(np.ceil(timesteps),999), torch.from_numpy( timesteps - np.floor(timesteps))
        # self.alphas_cumprod = self.alphas_cumprod.to('cpu')
        # self.sigmas = ((1 - alphas_cumprod) / alphas_cumprod) ** 0.5
        tmp_alphas = self.log_alphas.exp().to('cpu')
        alphas = (1 - w) * tmp_alphas[low_idx] + w * tmp_alphas[high_idx]
        low_alpha_cumprod = self.alphas_cumprod[low_idx].to('cpu')
        alpha_cumprod = low_alpha_cumprod * alphas
...
def model_wrapper(
    ...
    def get_x0_from_noise(x, t_continuous, noise):
        if t_continuous.reshape((-1,)).shape[0] == 1:
            t_continuous = t_continuous.expand((x.shape[0]))
        t_input = get_model_input_time(t_continuous)
        alpha_t, sigma_t = noise_schedule.marginal_alpha(t_continuous), noise_schedule.marginal_std(t_continuous)
        dims = x.dim()
        # tnoise = (x - expand_dims(alpha_t, dims) * output) / expand_dims(sigma_t, dims)
        return (x - noise * expand_dims(sigma_t, dims)) / expand_dims(alpha_t, dims)
    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            ...
            if guidance_scale == 1.0 or unconditional_condition is None:
                ...
                if grather_feature_dict is not None and grather_feature_dict['cond_noise'] is not None:
                    grather_feature_dict['cond_noise'].append(noise)
                if grather_feature_dict is not None and grather_feature_dict['cond_xstart'] is not None:
                    grather_feature_dict['cond_xstart'].append(get_x0_from_noise(x, t_continuous, noise))
                if grather_feature_dict is not None and grather_feature_dict['tmp_t'] is not None:
                    grather_feature_dict['tmp_t'].append(get_model_input_time(t_continuous))
                return noise
            else:
                x_in = torch.cat([x] * 2)
                ...
                if grather_feature_dict is not None and grather_feature_dict['cond_xstart'] is not None:
                    grather_feature_dict['cond_xstart'].append(get_x0_from_noise(x, t_continuous, noise))
                if grather_feature_dict is not None and grather_feature_dict['uncond_noise'] is not None:
                    grather_feature_dict['uncond_noise'].append(noise_uncond)
                if grather_feature_dict is not None and grather_feature_dict['uncond_xstart'] is not None:
                    grather_feature_dict['uncond_xstart'].append(get_x0_from_noise(x, t_continuous, noise_uncond))
                if grather_feature_dict is not None and grather_feature_dict['intermediate_x'] is not None:
                    grather_feature_dict['intermediate_x'].append(x)
                if grather_feature_dict is not None and grather_feature_dict['tmp_t'] is not None:
                    grather_feature_dict['tmp_t'].append(get_model_input_time(t_continuous))
