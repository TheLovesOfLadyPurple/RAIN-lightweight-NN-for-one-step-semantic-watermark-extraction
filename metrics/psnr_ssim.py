...
def calculate_psnr(img1,
    ...
    def _psnr(img1, img2):
        ...
        if mse == 0:
            return float('inf')
        max_value = img1.max() * 2 # 1. if img1.max() <= 1 else 255.
...
def calculate_L1Losses(img1,
    ...
    def _l1loss(img1, img2):
        mse = np.mean((img1 - img2)**2)
        if mse == 0:
            return float('inf')
        max_value = 2 # 1. if img1.max() <= 1 else 255.
        return np.sqrt(mse)
    
    return _l1loss(img1, img2)
...
def calculate_ssim(img1,
    ...
    if img1.ndim == 3 and img1.shape[2] == 8:
        l1, r1 = img1[:,:,:4], img1[:,:,4:]
        l2, r2 = img2[:,:,:4], img2[:,:,4:]
