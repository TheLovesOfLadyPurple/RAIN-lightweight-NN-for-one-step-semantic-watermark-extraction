"""Compare FARI, OSI, and Ours on SD 2.1 Gaussian-Shading image attacks."""
...
DEFAULT_OPTIONS = PROJECT_ROOT / 'options' / 'test' / 'cocoGSReverseDistillUNetAdvSDV21.yml'
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Compare FARI, OSI, and Ours on SD 2.1 GS images under all attacks.')
    parser.add_argument(
        '--opt', default=str(DEFAULT_OPTIONS),
    ...
    parser.add_argument(
        '--outdir', default=str(PROJECT_ROOT / 'outputs' / 'coco_gs_fari_sdv21'))
    parser.add_argument(
        '--generation-cache-dir',
        default=str(PROJECT_ROOT / 'outputs' / 'coco_generation_cache_sdv21'),
...
@torch.no_grad()
def osi_one_step_inversion(model, attacked_image, prompt_embeds, device, dtype):
    image = pil_to_tensor(attacked_image, device).unsqueeze(0) * 2 - 1
    timestep = torch.full((image.shape[0],), 999, device=device, dtype=torch.long)
...
def main():
    ...
    results = []
    progress_bar = tqdm(captions, unit='sample', desc='COCO SD 2.1 GS evaluation')
    for index, caption_record in enumerate(progress_bar):
        ...
        for attack_index, attack_name in enumerate(ATTACK_NAMES):
            ...
            attacks[attack_name] = {}
            if 'fari' in enabled_extractors:
                predicted_noise = one_step_inversion(pipe, attacked_latent, fari_null_prompt_embedding)
            ...
            if 'osi' in enabled_extractors:
                predicted_noise = osi_one_step_inversion(
                    osi_model, attacked_image, osi_null_prompt_embedding, device, dtype)
                watermark_result = watermark.evaluate(predicted_noise)
                attacks[attack_name]['osi'] = {
                    **watermark_result.to_dict(),
                    'noise_metrics': latent_metrics(predicted_noise, watermarked_noise),
                    'prediction_diagnostics': prediction_diagnostics(
                        predicted_noise, watermarked_noise),
            ...
            if 'ours' in enabled_extractors:
                ours_result = reverse_noise(model, attacked_latent, prompt_embedding)
                ours_watermark_result = watermark.evaluate(ours_result['predicted_noise'])
                attacks[attack_name]['ours'] = {
                    **ours_watermark_result.to_dict(),
                    'noise_metrics': latent_metrics(
                        ours_result['predicted_noise'], watermarked_noise),
                    'prediction_diagnostics': prediction_diagnostics(
                        ours_result['predicted_noise'], watermarked_noise),
                }
        results.append({
    ...
    aggregates = {
        name: aggregate_attack_results(results, name) for name in enabled_extractors}
    report = {
        'configuration': {
            ...
            'protocol_note': (
                'FARI and OSI are evaluated with their native SD 2.1-base components. '
    ...
    print(f'Saved SD 2.1 robustness results to {output_dir}')
