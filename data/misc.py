...
def check_resume(opt, resume_iter):
    ...
    if opt['path']['resume_state']:
        ...
        for network in networks:
            ...
            name_ema = f'pretrain_{network}_ema'
            ...
            if opt['path'].get('ignore_resume_networks') is None or (
                ...
                opt['path'][name_ema] = osp.join(
                    opt['path']['models'], f'net_{basename}_ema_{resume_iter}.pth')
                logger.info(f"Set {name_ema} to {opt['path'][name_ema]}")

        networks = [key for key in opt.keys() if key.startswith('lossconfig')]
        if network is not None: # wrong, for gan used
            name = 'pretrain_network_d'
            basename = 'd'#network.replace('network_', '')
            if opt['path'].get('ignore_resume_networks') is None or (
                    basename not in opt['path']['ignore_resume_networks']):
                opt['path'][name] = osp.join(
                    opt['path']['models'], f'net_{basename}_{resume_iter}.pth')
